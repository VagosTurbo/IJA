...
public class Room implements Environment {
    ...
    public boolean createObstacleAt(Position p) {
        if (this.containsPosition(p) && !this.obstacleAt(p) && !this.robotAt(p)) {
    ...
    public boolean robotAt(Position p) {
        for (Robot robot : getRobots()) {
                ...
                double distance = Math.sqrt(Math.pow(p.getX() - robotX, 2) + Math.pow(p.getY() - robotY, 2));
                if (distance < ROBOT_SIZE + 5) {
                    return true;
            }
        }
        return false;
    ...
    public void load(JsonObject json){
        try {
            ...
            for (JsonElement element : robotArray) {
                ...
                double speed = robotObject.get("speed").getAsDouble();

                if (controlled) {
                    Robot robot = ControlledRobot.create(this, new Position(x, y), id, speed);
                    if (robot != null) {
                        this.robots.add(robot);
                    }
                } else {
                    Robot robot = AutonomousRobot.create(this, new Position(x, y), 1, speed, true, id, turnAngle, currentAngle);
                    if (robot != null) {
                        this.robots.add(robot);
                    }
                }
